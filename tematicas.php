...
<body>
    ...
    <main>
        <?php
            $response = json_decode(file_get_contents('http://localhost/ProyectosK/Shop/api/productos/api-productos.php?categoria=tematica'), true);
            if($response['statuscode'] == 200){
                foreach($response['items'] as $item){?>
                    <tr>
                        ...
                        <td> <?php if($item['foto6']!=''){?> <img witdth="50px" height="50px" src="img/<?php echo $item['foto6']; ?>"><?php ; } ?></td>
                    </tr>
               <?php }
            }else{
